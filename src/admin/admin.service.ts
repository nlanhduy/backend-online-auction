...
export class AdminService{
    ...
    private async getRevenueStats(dateFilter:any){
        ...
        const totalRevenue=Math.round(orders.reduce((sum, order)=>sum+(order.paymentAmount||0), 0) * 100) / 100;
        const platformRevenue=Math.round(orders.reduce((sum, order)=>sum+(order.platformFee||0), 0) * 100) / 100;
    ...
    private async getTopSellers(limit: number = 5) {
        ...
        .map((seller) => ({
            ...
            revenue: Math.round(seller.Order_Order_sellerIdToUser.reduce(
    ...
    private async getTopBidders(limit: number = 5) {
        ...
        .map((bidder) => ({
            ...
            totalSpent: Math.round(bidder.Order_Order_buyerIdToUser.reduce(
            (sum, o) => sum + (o.paymentAmount || 0),
            0,
            ) * 100) / 100,
        }))
