...
export class PaymentController {
  ...
  @ApiOperation({
    ...
    description:
      'Create PayPal order for won auction product. Returns approval URL to redirect user to PayPal for payment.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Capture payment after user approves on PayPal. Call this API after user returns from PayPal. Automatically creates Order record after successful payment.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'This endpoint is called by PayPal after user approves payment. Shows instructions to complete the payment capture.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Endpoint to receive notifications from PayPal about payment status (PAYMENT.CAPTURE.COMPLETED, DENIED, REFUNDED, etc.)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Refund full or partial amount for a captured payment. If amount is not provided, full refund will be processed.',
  })
  @ApiParam({
    name: 'captureId',
    description: 'PayPal Capture ID (transaction ID)',
    example: '8YY98765WXYZ...',
  })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        amount: {
          type: 'number',
          description: 'Refund amount (USD). Leave empty for full refund',
          example: 50.0,
        },
      },
    },
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async refundPayment(@Param('captureId') captureId: string, @Body() body: { amount?: number }) {
    try {
      const result = await this.paymentService.refundPayment(captureId, body.amount);
