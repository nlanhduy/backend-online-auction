...
export class PaypalService {
  ...
  async createOrder(
    amount: number,
    currency: string = 'USD',
    description: string = 'Auction Payment',
  ) {
  ...
  async payoutToSeller(sellerEmail: string, amount: number, orderId: string) {
    ...
    try {
      const response = await axios.post(`${this.paypalApiUrl}/v1/payments/payouts`, payoutData, {
        headers: {
      ...
      return {
        ...
        itemId: response.data.links?.[0]?.href,
      };
    } catch (error) {
      console.error('PayPal Payout error:', error.response?.data || error);
      throw new Error(
        `Failed to payout to seller: ${error.response?.data?.message || error.message}`,
      );
    }
  }
  ...
  async getPayoutStatus(batchId: string) {
    ...
    try {
      const response = await axios.get(`${this.paypalApiUrl}/v1/payments/payouts/${batchId}`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
        },
